...
def ingest_recording(recording_path):
    ...
    exists_recording = bagman_utils.db_exists_recording(
        args.recording_name, config["database_path"]
    )
    if exists_recording:
...
if __name__ == "__main__":
    ...
    if args.command == "upload":
        ...
        if os.path.exists(os.path.join(config["recordings_storage"], recording_name)):
            if not click.confirm(
                "Recording already exists. Do you want to override it?", default=True
            ):
                print("Operation cancelled.")
                exit(0)
        ...
        if not click.confirm(
            "Are you sure you want to delete the recording?", default=False
        ):
            print("Operation cancelled.")
            exit(0)
